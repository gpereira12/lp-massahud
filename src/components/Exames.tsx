...
const Exames = () => {
  ...
  const gestacaoExams = [
    { title: "Obstétrico Convencional", icon: Baby, description: "Acompanhamento essencial do desenvolvimento fetal em todas as fases da gestação via Doppler." },
    { title: "Obstétrico 1º Trimestre", icon: Search, description: "Confirmação de vitalidade e datação precisa logo nas primeiras semanas com análise circulatória." },
    { title: "Translucência Nucal", icon: Dna, description: "Rastreamento genético vital com avaliação minuciosa da ducto venoso e osso nasal via Doppler." },
    { title: "Morfológico 1º Trimestre", icon: Microscope, description: "Detecção precoce de malformações com estudo de artérias uterinas para predição de pré-eclâmpsia." },
    { title: "Morfológico 2º Trimestre", icon: Scan, description: "O check-up fetal mais completo para avaliação de todos os órgãos e ecocardiografia funcional." },
    { title: "Obstétrico com Doppler", icon: Activity, description: "Monitoramento hemodinâmico rigoroso para garantir que a nutrição do seu bebê esteja perfeita." },
    { title: "Perfil Biofísico Fetal", icon: HeartPulse, description: "Avaliação da vitalidade fetal através de movimentos, tônus e dinâmica do fluxo sanguíneo." }
  ];

  const generalExams = [
    { title: "Ultrassom Endovaginal", icon: Flower2, description: "Mapeamento detalhado do útero e ovários com Doppler colorido para diagnóstico de pólipos ou cistos." },
    { title: "Mamas e Axilas", icon: Shield, description: "Prevenção com Doppler para avaliar a vascularização de nódulos, garantindo laudos mais precisos." },
    { title: "Abdome Total", icon: Layers, description: "Estudo hemodinâmico de órgãos como fígado, rins e grandes vasos abdominais para diagnósticos seguros." },
    { title: "Aparelho Urinário", icon: Droplets, description: "Análise funcional de rins e bexiga, detectando obstruções e calculose com auxílio do Doppler." },
    { title: "Tireóide", icon: Radio, description: "Classificação precisa de nódulos através do estudo da vascularização central e periférica." }
  ];

  const categories = [
    { id: "baby", label: "Acompanhamento da Gestação", icon: Baby },
  ...
  return (
    <section id="exames" className="bg-white pt-24 pb-16 px-6 lg:px-16 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto relative z-20">
        <div className="text-center mb-20 animate-fade-in-up">
          <div className="inline-flex items-center gap-3 bg-[var(--accent-gold)]/10 px-6 py-2 rounded-full mb-8 border border-[var(--accent-gold)]/20 shadow-sm">
            ...
            <span className="uppercase text-[var(--accent-gold)] text-xs tracking-widest font-black font-sans">
              Diferencial Doppler em 100% dos exames
            </span>
          </div>
        ...
        <div ref={sectionRef} className="sticky top-[70px] z-40 py-4 mb-16 bg-white/80 backdrop-blur-sm -mx-6 px-6">
          <div className="max-w-fit mx-auto bg-white/60 backdrop-blur-md p-1.5 rounded-[2.5rem] shadow-xl border border-slate-100 flex gap-1 overflow-x-auto no-scrollbar scroll-smooth">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => handleTabChange(cat.id as any)}
                className={`
                  relative flex items-center gap-2 sm:gap-3 px-6 sm:px-10 py-4 sm:py-5 rounded-[2rem] text-xs sm:text-sm font-bold font-sans transition-all duration-500 whitespace-nowrap
              ...
              >
                {activeTab === cat.id && (
                  <motion.div 
                    layoutId="activeTab"
                    className="absolute inset-0 bg-gradient-to-r from-[#A69D24] to-[#C9C12C] rounded-[2rem] shadow-lg shadow-[#A69D24]/20"
              ...
              >
                ...
                <div className="relative z-10">
                  <WhatsAppButton 
                    ...
                    className="!bg-[var(--primary-blue)] !text-white hover:!bg-[#1a355d] border-none !px-8 !py-4 !text-sm flex gap-2 items-center !shadow-none !rounded-xl w-full"
                  >
